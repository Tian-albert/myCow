...
class UserDAO:
    ...
    def create_user(self, wx_id: str, nickname: str = None, gender: int = 0,
                    height: float = None, weight: float = None, age: int = 22) -> int:
        """
        ...
        sql = """
            INSERT INTO user (wx_id, nickname, gender, height, weight, age)
            VALUES (?, ?, ?, ?, ?, ?)
        """
        cur.execute(sql, (wx_id, nickname, gender, height, weight, age))
    ...
    def get_user_by_wx_id(self, wx_id: str) -> User:
        ...
        sql = """
            SELECT user_id, wx_id, nickname, height, weight, gender, age
            FROM user
            WHERE wx_id = ?
        """
    ...
    def update_user_info(self, wx_id: str, nickname: str = None,
                         ...
                         weight: float = None, age: int = None) -> bool:
        """
        更新用户信息；返回是否有行受到影响
        """
        conn = self.db_manager.get_conn()
