        ...
        if ctype == ContextType.TEXT:
            ...
            if context.get("isgroup", False):  # 群聊
                ...
                if context["msg"].to_user_id != context["msg"].actual_user_id:
                    ...
                    if context["msg"].is_at and context["msg"].to_user_id in context['msg'].at_list : #at 的是机器人，并且是at消息
    ...
    def _generate_reply(self, context: Context, reply: Reply = Reply()) -> Reply:
        ...
        if AI_reply and not e_context.is_pass():
            ...
            if context.type  in [ContextType.TEXT ,ContextType.IMAGE_CREATE,ContextType.QUOTE ]:  # 文字和图片消息
                ...
                msg = context["msg"]
                uuid = msg._rawmsg.get("uuid") or msg.msg_id
                uuid = str(uuid)
                session_id = context["session_id"]
                if not uuid:
                    logger.warning("[WX] No uuid found in image message")
                    return
                chat_id = msg.other_user_id if msg.is_group else msg.from_user_id
                memory.save_msg_to_cache(uuid, context["msg"].content, "image", chat_id, 
                                       session_id=session_id, msg=context.get("msg"))
                ...
                session_id = context["session_id"]
                if not uuid:
                    logger.warning("[WX] No uuid found in file message")
                    return
                chat_id = msg.other_user_id if msg.is_group else msg.from_user_id
                memory.save_msg_to_cache(uuid, context["msg"].content, "file", chat_id,
                                       session_id=session_id, msg=context.get("msg"))
                pass
                ...
                session_id = context["session_id"]
                if not uuid:
                    logger.warning("[WX] No uuid found in emoji message")
                    return
                chat_id = msg.other_user_id if msg.is_group else msg.from_user_id
                memory.save_msg_to_cache(uuid, context["msg"].content, "emoji", chat_id,
                                       session_id=session_id, msg=context.get("msg"))
                ...
                session_id = context["session_id"]
                if not uuid:
                    logger.warning("[WX] No uuid found in video message")
                    return
                chat_id = msg.other_user_id if msg.is_group else msg.from_user_id
                memory.save_msg_to_cache(uuid, context["msg"].content, "video", chat_id,
                                       session_id=session_id, msg=context.get("msg"))
