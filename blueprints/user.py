...
def get_qrcode_route(account_id):
    ...
    if account.account_type == 0:
        ...
        # 创建新的wxbot实例
        wxbot = iPadWx()
        wxbot.app_id = account.token
        wxbot.token = account.auth
        if wxbot.app_id is None:
            wxbot.app_id = ""
        # status = check_wechat_status_beta(wxbot)
        # if status:
        #     return jsonify({'info': '获登录二维码失败！已经登录了'})
        # else:# 可能已掉线，但显示已登录
        #     wxbot.log_out()
        # 获取二维码（这个操作不需要完整初始化）
        if wxbot.is_login==False:
            wxbot.log_out()
        qr_response = wxbot.get_qrcode()


        if qr_response and qr_response.get('ret')==500:
            if qr_response.get('msg')=="微信已登录，请勿重复登录。":
                return jsonify({
                    'qrcode_data': "",
                    'msg':qr_response.get('msg'),
                    'is_wx_token': True
                })
            else:
                wxbot.log_out()
        ...
        if qr_response and qr_response.get('data') and  qr_response['data'].get('qrImgBase64'):
            qrcode_data = qr_response['data'].get('qrImgBase64')
            # 缓存wxbot实例
            wxbot.app_id = qr_response['data'].get('appId') # 获取appId 然后保存到数据库
            wx_bot_instances[account_id] = wxbot
            return jsonify({
...
def check_wechat_status_beta(wxbot):
    ...
    if status_check.get('ret',0) != 200 or not status_check.get('data'):
        return False
    
    # Second check - try sending a message
    test_msg = wxbot.send_txt_msg("filehelper", "Connection test")
    if test_msg.get('ret') != 200 or test_msg.get("code") == 0 :
        return False
    ...
    if response and response.get("data"):
        ...
        if status == 2:  # Login confirmed
            ...
            if account:
                ...
                account.token = app_id
                account.account_id = login_info['alias']
...
@user_bp.route('/update_wechat_account', methods=['GET', 'POST'])
async def update_wechat_account():
    if request.method == 'GET':
        try:
            ...
            return jsonify({'error': '未找到对应的账号'}), 404
        except Exception as e:
    ...
    # POST 方法保持不变
        ...
        try:
            ...
            if account:
                account.auth_account = data.get('auth_account', account.auth_account)
                account.auth_password = data.get('auth_password', account.auth_password)
                account.token = data.get('token', account.token)
                account.auth = data.get('auth', account.auth)
...
async def startlisten():
    ...
    if account:
        ...
        if account.callback_url:
            ...
            logger.info(result)
            if result and (result.get("code") == 0 or result.get("ret") == 200):
...
async def test_heartbeat():
    ...
    if account.account_type == 0:
        ...
        if result.get('ret') == 200 or result.get('code') == 0:
...
def wx_login_gewe(account):
    from channel.wechat.iPadWx_Beta import iPadWx
    wxbot = iPadWx()
    wxbot.app_id = account.get("token")
    wxbot.token = account.get("auth")

    # Check if truly online
    is_online = check_wechat_status(wxbot)
    
    if is_online:
        return jsonify({'status': 'success', 'message': '微信在线，不需要扫码'})
    
    # Get QR code if offline
    qrcode_data = get_login_qrcode(wxbot)
    if qrcode_data:
        return jsonify({'qrcode_data': qrcode_data})
    
    return jsonify({'status': 'error', 'message': '获取登录二维码失败'})
